...
function SOEProxy(protocolName, remoteAddress, remotePort, cryptoKey, localPort, localClientPort) {
    ...
    server.on("appdata", function(err, proxyClient, data) {
        log.debug("[SOEProxy]", "Received app data from proxy client");
        var obj = {
            appdata: data
        };
    ...
    client.on("appdata", function(err, data) {
        log.debug("[SOEProxy]", "Received app data from remote server");
        var obj = {
            appdata: data
        };
        ...
        if (proxyClient) {
            ...
            log.debug("[SOEProxy]", "No client yet, dropping app data packet");
        }
    });
}
