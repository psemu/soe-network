...
function packSOEPacket(packetName, object, crcSeed, compression, isSubPacket) {
    ...
    if (packet) {
        if (packet.pack) {
            ...
            log.debug("[SOEProtocol]", "Packing data for " + packet.name);
        } else {
            log.debug("[SOEProtocol]", "pack()", "No pack function for packet " + packet.name);
        }
    } else {
        log.warn("[SOEProtocol]", "pack()", "Unknown or unhandled SOE packet type: " + packetType);
    }
    return data;
}
...
function parseSOEPacket(data, crcSeed, compression, isSubPacket, appData) {
    ...
    if (packet) {
        if (packet.parse) {
            log.debug("[SOEProtocol]", packet.name);
            ...
            log.debug("[SOEProtocol]", "parse()", "No parser for packet " + packet.name);
        }
    } else {
        log.warn("[SOEProtocol]", "parse()", "Unknown or unhandled SOE packet type: " + packetType);
    }
}
