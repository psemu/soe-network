...
function TransparentProxy(remoteAddress, remotePort, localPort, localClientPort) {
    ...
    server.on("message", function(data, remote) {
        ...
        log.debug("[TransparentProxy]", "Client -> Server (" + data.length + ")");
        client.send(
            data, 0, data.length, 
    ...
    client.on("message", function(data, remote) {
        if (!proxyClient) {
            log.error("[TransparentProxy]", "Received data from server but client is not ready");
        }
        log.debug("[TransparentProxy]", "Server -> Client (" + data.length + ")");
        server.send(
            data, 0, data.length, 
    ...
    client.on("listening", function () {
        ...
        log.info("[TransparentProxy]", "Listening for remote server on " + address.address + ":" + address.port);
    });
    server.on("listening", function () {
        var address = this.address();
        log.info("[TransparentProxy]", "Listening for local client on " + address.address + ":" + address.port);
    });
}
