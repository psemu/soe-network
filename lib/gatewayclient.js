...
GatewayClient.prototype.connect = function(callback) {
    log.info("[GatewayClient]", "Connecting to gateway server");
...
GatewayClient.prototype.sendTunnelData = function(tunnelData, channel) {
    channel = channel || 0;
    log.info("[GatewayClient]", "Sending tunnel data to gateway server");
...
GatewayClient.prototype.login = function(characterId, ticket, clientProtocol, clientBuild) {
    log.info("[GatewayClient]", "Sending login request");
