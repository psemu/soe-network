...
function SOEClient(protocolName, serverAddress, serverPort, cryptoKey, localPort) {
    ...
    log.debug("[SOEClient]", this._guid, "Creating new SOEClient instance");
    ...
    inputStream.on("data", function(err, data) {
        if (me._dumpData) {
            fs.writeFileSync("debug/soeclient_apppacket_" + (n2++) + ".dat", data);
        }
    ...
    function checkOutOfOrderQueue() {
        if (outOfOrderPackets.length) {
            ...
            log.info("[SOEClient]", "Sending " + packets.length + " OutOfOrder packets");
    ...
    function handlePacket(packet) {
        ...
        switch (soePacket.name) {
            case "SessionReply":
                log.debug("[SOEClient]", "Received session reply from server");
                ...
                break;
            case "Disconnect":
                log.debug("[SOEClient]", "Received disconnect from server");
                me.disconnect();
                me.emit("disconnect");
                break;
            case "MultiPacket":
                ...
                if (lastOutOfOrder > 0) {
                    log.info("[SOEClient]", "Received multiple out-order-packet packet on channel " + channel + ", sequence " + lastOutOfOrder);
                    outputStream.resendData(lastOutOfOrder);
                }
                break;
            case "Ping":
                log.debug("[SOEClient]", "Received ping from server");
                break;
            case "NetStatusReply":
                log.debug("[SOEClient]", "Received net status reply from server");
                break;
            case "Data":
                log.debug("[SOEClient]", "Received data packet from server");
                inputStream.write(result.data, result.sequence, false);
                break;
            case "DataFragment":
                log.debug("[SOEClient]", "Received data fragment from server");
                inputStream.write(result.data, result.sequence, true);
                break;
            case "OutOfOrder":
                log.info("[SOEClient]", "Received out-order-packet packet on channel " + result.channel + ", sequence " + result.sequence);
            ...
            case "Ack":
                ...
                break;
            case "FatalError":
                log.debug("[SOEClient]", "Received fatal error from server");
                break;
    ...
    connection.on("listening", function () {
        ...
        log.info("[SOEClient]", "Listening on " + address.address + ":" + address.port);
    });
...
SOEClient.prototype.connect = function() {
    log.debug("[SOEClient]", "Setting up connection for " + this._serverAddress + ":" + this._serverPort);
...
SOEClient.prototype.toggleEncryption = function(value) {
    ...
    log.debug("[SOEClient]", this._guid, "Toggling encryption: value = " + value);
...
SOEClient.prototype._sendPacket = function(packetName, packet, prioritize) {
    ...
    log.debug("[SOEClient]", this._guid, "Sending " + packetName + " packet to server");
    if (this._dumpData) {
...
SOEClient.prototype.sendAppData = function(data, overrideEncryption) {
    log.debug("[SOEClient]", this._guid, "Sending app data: " + data.length + " bytes");
