...
function SOEError(message) {
    this.name = this.constructor.name;
    this.message = message;
}
util.inherits(SOEError, Error);
...
function SOEServer(protocolName, serverPort, cryptoKey, compression) {
    ...
    function handlePacket(client, packet) {
        ...
        switch (soePacket.name) {
            case "SessionRequest":
                log.debug("[SOEServer]", "Received session request from " + client.address);
                ...
                break;
            case "Disconnect":
                log.debug("[SOEServer]", "Received disconnect from client");
            ...
            case "MultiPacket":
                ...
                if (lastOutOfOrder > 0) {
                    log.info("[SOEServer]", "Received multiple out-order-packet packet on channel " + channel + ", sequence " + lastOutOfOrder);
                    client.outputStream.resendData(lastOutOfOrder);
                }
                break;
            case "Ping":
                log.debug("[SOEServer]", "Received ping from client");
                break;
            case "NetStatusRequest":
                log.debug("[SOEServer]", "Received net status request from client");
                break;
            case "Data":
                log.debug("[SOEServer]", "Received data packet from client");
                client.inputStream.write(result.data, result.sequence, false);
                break;
            case "DataFragment":
                log.debug("[SOEServer]", "Received data fragment from client");
                client.inputStream.write(result.data, result.sequence, true);
                break;
            case "OutOfOrder":
                log.info("[SOEServer]", "Received out-order-packet packet on channel " + result.channel + ", sequence " + result.sequence);
            ...
            case "Ack":
                ...
                break;
            case "FatalError":
                log.debug("[SOEServer]", "Received fatal error from client");
                break;
        ...
        if (!clients[remote.address]) {
            ...
            var checkOutOfOrderQueue = function() {
                if (client.outOfOrderPackets.length) {
                    ...
                    log.info("[SOEServer]", "Sending " + packets.length + " OutOfOrder packets");
    ...
    connection.on("listening", function () {
        ...
        log.info("[SOEServer]", "Listening on " + address.address + ":" + address.port);
    });
...
SOEServer.prototype.sendAppData = function(client, data, overrideEncryption) {
    log.debug("[SOEServer]", "Sending app data: " + data.length + " bytes");
...
SOEServer.prototype.toggleEncryption = function(value) {
    ...
    log.debug("[SOEServer]", this._guid, "Toggling encryption: value = " + value);
